...
export async function scheduleSuspend(workspaceId) {
...
export async function cancelSuspend(workspaceId) {
    await chrome.alarms.clear(SUSPEND_ALARM_PREFIX + workspaceId);
}
export function isSuspendAlarm(alarmName) {
...
export async function onWorkspaceSwitched(previousWorkspaceId, workspaceId) {
...
export async function rescheduleAllSuspends() {
    const state = await getState();
